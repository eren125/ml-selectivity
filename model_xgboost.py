...
T_h = 900
df_data['G_900K'] = -R*T_h*np.log(df_data['KH_900K_xenon']/df_data['KH_900K_krypton'])
...
# %%
data_dmatrix = xgb.DMatrix(data=X.to_numpy(),label=y.to_numpy(), feature_names=X_columns)

cv_results = xgb.cv(dtrain=data_dmatrix, params=params, nfold=5,num_boost_round=num_boost_round,early_stopping_rounds=10,metrics="rmse", as_pandas=True, seed=random_state)
...
fig = plt.figure(constrained_layout=True, figsize=(5, 4))
ax = fig.subplots(1, 1)
ax.plot([0, 1], [cv_results.iloc[-1,2]-0.005, cv_results.iloc[-1,2]-0.005], transform=ax.transAxes, linestyle="--", alpha=0.8, color = "gray")
cv_results.reset_index().plot(y='train-rmse-mean', x='index', ax=ax)
cv_results.reset_index().plot(y='test-rmse-mean', x='index',ax=ax)
...
plt.savefig('plot/convergence_plot.pdf', dpi=240, bbox_inches='tight')
plt.show()
...
# %%
filename = 'model/20230502_final_selectivity_model.sav'
